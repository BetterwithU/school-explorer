...
### 이미지 / base64 (MUST)
- **이미지를 base64로 변환해 코드·HTML·대화에 인라인 금지.** 꼭 필요할 때만 사용한다.
  - 이유: base64는 원본보다 ~33% 커지고, 파일을 통째로 컨텍스트/코드에 박아 무겁게 만든다. 큰 이미지는 API 요청 한도(5MB)를 터뜨려 작업이 반복 실패한다.
  - 기본: 이미지는 `<img src="images/...">`처럼 **파일 경로 참조**로만 쓴다.
  - base64가 정당한 예외(작은 아이콘 인라인이 꼭 필요한 특수 상황) → 사용자에게 먼저 확인.
- 게임에 쓰는 이미지는 **모바일 우선으로 경량화** (WebP/적정 해상도, 가급적 수백 KB대). 모바일 대응·최적화가 최우선 순위.

### 검증 (MUST)
- **검증한답시고 프로젝트 파일을 변형·생성하지 말 것.** (`_tmp_inject.html` 같은 임시 주입 파일, 진행상태 심는 더미 HTML 등 절대 금지)
- **검증한답시고 헤드리스 스크린샷을 찍어 Read로 읽지 말 것.** PNG가 base64로 부풀어 API 5MB 한도를 터뜨려 세션이 반복적으로 뻗는 주범이다.
- 좌표·로직 검증은 **코드를 직접 읽어 추론**한다. 시각 확인이 필요하면 **사용자가 로컬/배포본에서 직접** 보고 피드백한다.
